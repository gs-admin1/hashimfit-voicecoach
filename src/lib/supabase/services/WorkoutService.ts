...
export class WorkoutService {
  ...
  // Add new method for updating all future workouts of the same type
  static async updateAllFutureWorkoutsOfType(
    userId: string, 
    workoutPlanId: string, 
    exercises: Omit<WorkoutExercise, 'id' | 'workout_plan_id'>[]
  ): Promise<boolean> {
    try {
      console.log(`Updating all future workouts for plan ${workoutPlanId}`);
      
      // Get the workout plan details
      const workoutPlan = await this.getWorkoutPlanById(workoutPlanId);
      if (!workoutPlan) {
        console.error("Workout plan not found");
        return false;
      }

      // Delete all existing exercises for this workout plan
      const existingExercises = await this.getWorkoutExercises(workoutPlanId);
      await Promise.all(
        existingExercises.map(ex => this.deleteWorkoutExercise(ex.id!))
      );

      // Create new exercises with updated data
      ...
      const result = await this.createWorkoutExercises(exerciseData);
      
      if (result) {
        console.log(`Successfully updated all future workouts for plan ${workoutPlanId}`);
        return true;
      }
      
      return false;
    } catch (error) {
      console.error('Error updating all future workouts of type:', error);
      return false;
    }
  }

  // Add method to create a one-off workout schedule with custom exercises (for today-only saves)
  static async createOneOffWorkoutSchedule(
    userId: string,
    ...
    try {
      console.log(`Creating one-off workout for date ${scheduledDate}`);
      
      // Get the original workout plan
      const originalPlan = await this.getWorkoutPlanById(originalWorkoutPlanId);
      if (!originalPlan) {
        console.error("Original workout plan not found");
        return null;
      }

      // Create a new workout plan with modified exercises (copy of original)
      const oneOffPlan: WorkoutPlan = {
        user_id: userId,
        title: `${originalPlan.title} (Custom - ${scheduledDate})`,
        description: `One-off modification of ${originalPlan.title}`,
      ...
      const createdPlan = await this.createWorkoutPlan(oneOffPlan);
      if (!createdPlan || !createdPlan.id) {
        console.error("Failed to create one-off workout plan");
        return null;
      }
      ...
      await this.createWorkoutExercises(exerciseData);

      // Update the existing schedule to use the new workout plan
      const { error: updateError } = await supabase
        .from('workout_schedule')
        .update({ workout_plan_id: createdPlan.id })
        .eq('user_id', userId)
        .eq('scheduled_date', scheduledDate)
        .eq('workout_plan_id', originalWorkoutPlanId);

      if (updateError) {
        console.error("Error updating workout schedule:", updateError);
        return null;
      }

      console.log(`Successfully created one-off workout plan ${createdPlan.id}`);
      return createdPlan.id;
    } catch (error) {
      console.error('Error creating one-off workout schedule:', error);
      return null;
    }
  }
}
